...
void printf_yolo_detections(char **id, int total, int classes, int w, int h, detection *dets)
{
    int i, j;
    for(i = 0; i < total; ++i){
        float xmin = dets[i].bbox.x - dets[i].bbox.w/2.;
...
void test_yolo(char *cfgfile, char *weightfile, char *filename, float thresh)
{
	...
	//printf("weight:%s,cfgfile:%s\n",weightfile,cfgfile);
    network *net = load_network(cfgfile, weightfile, 0);
    //network* net = load_network_train(cfgfile, weightfile, 0);
	//printf("net is Ok\n");
    //printf("layersize: %d", sizeof(layer));
    layer l = net->layers[net->n-1];
    set_batch_network(net, 1);
	//printf("batch network is Ok\n");
    srand(2222222);
    clock_t time;
    char buff[256];
    char *input = buff;
...
//#define OUTPROGRESS
        ...
        float *X = sized.data;
        time=clock();
		//printf("resize image is Ok\n");
        network_predict(net, X);
        int nboxes = 0;
        detection *dets = get_network_boxes(net, 1, 1, thresh, 0, 0, 0, &nboxes);
        if (nms) do_nms_sort(dets, nboxes, l.classes, nms);
        draw_detections(im, dets, nboxes, thresh, voc_names, alphabet, 20);
        ...
        int iend = 200;
        clock_t st = clock();
        for (int cnum = 0; cnum <iend;cnum++ )
        {
#ifdef OUTPROGRESS
            printf("net predict :%d start\n", cnum);
#endif
            network_predict_image(net, im);
#ifdef OUTPROGRESS
            printf("net work predict end\n");
#endif
            int nboxes = 0;
		...
		//printf_yolo_detections(voc_names,nboxes,10,net->w,net->h,dets);
		//printf("Before drawDetection:%d %d\n",l.side*l.side*l.n,nboxes);
