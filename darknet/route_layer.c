...
#include <stdio.h>
route_layer make_route_layer_CV(int batch, int n, int *input_layers, int *input_sizes)
{
    ...
    l.outputs = outputs;
    l.inputs = outputs;
    l.delta = 0;//CV// calloc(outputs*batch, sizeof(float));
    l.output = 0;//CV//calloc(outputs*batch, sizeof(float));;
    ...
    l.delta_gpu = 0;//CV// cuda_make_array(l.delta, outputs*batch);
    l.output_gpu =0;//CV// cuda_make_array(l.output, outputs*batch);
    #endif
    return l;
}
