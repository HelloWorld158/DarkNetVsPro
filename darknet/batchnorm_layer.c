...
{
    if(l.type == BATCHNORM) simple_copy_ongpu(l.outputs*l.batch, net.input_gpu,  l.output_gpu);    
    //CV/// copy_gpu(l.outputs * l.batch, l.output_gpu, 1, l.x_gpu, 1);原始位置
    if (net.train) {
#ifdef CUDNN
        float one = 1;
        float zero = 0;
        copy_gpu(l.outputs * l.batch, l.output_gpu, 1, l.x_gpu, 1);////CV//原来的位置没有这行代码
