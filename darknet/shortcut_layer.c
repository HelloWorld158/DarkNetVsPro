...
#include <stdio.h>
#include <assert.h>
layer make_shortcut_layer_CV(int batch, int index, int w, int h, int c, int w2, int h2, int c2)
{
    ...
    l.inputs = l.outputs;
#ifdef MEMORYDEBUG
    printf("Make ConnectLayer:%d\n", index);
#endif
    l.index = index;

    l.delta =0; //CV// calloc(l.outputs*batch, sizeof(float));
    l.output = 0;//CV//calloc(l.outputs*batch, sizeof(float));;
    ...
    l.delta_gpu = 0; //CV//cuda_make_array(l.delta, l.outputs*batch);
    l.output_gpu = 0;//CV//cuda_make_array(l.output, l.outputs*batch);
    #endif
    return l;
}
