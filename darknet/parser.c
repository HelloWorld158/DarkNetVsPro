...
convolutional_layer parse_convolutional(list *options, size_params params)
{
    ...
    //CV//convolutional_layer layer = make_convolutional_layer(batch,h,w,c,n,groups,size,stride,padding,activation, batch_normalize, binary, xnor, params.net->adam);
    convolutional_layer layer = make_convolutional_layer_CV_METHOD(GetConvolutionTrainMethod(), batch, h, w, c, n, groups, size, stride, padding, activation
...
network *parse_network_cfg(char *filename)
{
    ...
    params.net = net;
    NetWorkMemoryConfigerEx(&params, s, options,sections, net,n);
    return net;
}
...
void load_weights_predict_upto(network* net, char* filename, int start, int cutoff)
{
#ifdef GPU
    if (net->gpu_index >= 0) {
        cuda_set_device(net->gpu_index);
...
void load_weights_predict(network* net, char* filename)
{
    load_weights_predict_upto(net, filename, 0, net->n);
}